...
# fairseq preprocess refuses to overwrite an existing dict.txt from a partial run
rm -f $target_dir/dict.txt
...
rm -f $target_dir/phones/dict.txt
...
# Avoid duplicate <SIL> if the phone dict already contains it (fairseq rejects duplicates).
if ! grep -qE '^<SIL>[[:space:]]' $target_dir/phones/dict.phn.txt; then
...
# -s: allow ARPA without </s> when the phone corpus is tiny or degenerate (prefer fixing G2P/data upstream).
$KENLM_ROOT/build_binary -s $target_dir/phones/lm.phones.filtered.04.arpa $target_dir/phones/lm.phones.filtered.04.bin
...
$KENLM_ROOT/build_binary -s $target_dir/phones/lm.phones.filtered.06.arpa $target_dir/phones/lm.phones.filtered.06.bin
