...
class RealData(nn.Module):
    ...
    Responsibility: Given a sequence of token indices (phonemes drawn from
    unpaired text), convert them into one-hot probability vectors that the
    discriminator will treat as the 'real' distribution to match against.

    This class encapsulates the real-data side of the GAN, keeping it clearly
    separated from the Generator (fake side) and the Discriminator (judge).

    Args:
        output_dim (int): Size of the vocabulary / phoneme set. Each token is
                          represented as a one-hot vector of this dimension.
        pad_index (int):  Index used for padding tokens. Padding positions are
                          flagged in the returned padding mask so the
                          discriminator can ignore them.
    """

    def __init__(self, output_dim: int, pad_index: int):
        super().__init__()
    ...
    def forward(
        ...
        """
        Convert token indices into one-hot vectors.

        Args:
            tokens: LongTensor of shape (B, T) containing phoneme token indices
                    drawn from unpaired text data.

        Returns:
            token_x:      FloatTensor of shape (B, T, output_dim). Each
                          position is a one-hot vector over the vocabulary.
                          These are the 'real' samples for the discriminator.
            padding_mask: BoolTensor of shape (B, T). True where the token is
                          a padding index and should be ignored.
        """
        padding_mask = tokens == self.pad_index

        # Build one-hot matrix: (B*T, output_dim)
        token_x = tokens.new_zeros(tokens.numel(), self.output_dim).float()
        token_x.scatter_(1, tokens.reshape(-1, 1).long(), 1.0)

        # Reshape back to (B, T, output_dim)
        token_x = token_x.view(tokens.shape + (self.output_dim,))

        return token_x, padding_mask
    ...
    def __init__(self, dim, cfg: Wav2vec_UConfig):
        ...
        self.net = nn.Sequential(
            *emb_net,
            nn.Dropout(cfg.discriminator_dropout),
            *inner_net,
        ) #* is used to unpack the list into individual elements
...
class Generator(nn.Module):
    ...
    def forward(self, dense_x, dense_padding_mask, tokens=None):
        # ``tokens`` is unused (real phoneme samples come from ``RealData``); kept for API compatibility.
        result = {}
        ...
        # NOTE: token_x construction has been moved to RealData.
        # Generator is now solely responsible for producing fake samples.
...
class Wav2vec_U(BaseFairseqModel):
    ...
    def __init__(self, cfg: Wav2vec_UConfig, target_dict):
        ...
        # --- NEW: RealData instance ---
        # Encapsulates all logic for constructing the 'real' side of the GAN.
        # Previously this was inline inside forward(). It is now a first-class
        # component following the separation-of-concerns principle required by
        # the assignment.
        self.real_data = RealData(output_size, self.pad)
    ...
    def forward(
        ...
        #  real samples now come from self.real_data
        # Previously token_x and token_padding_mask were constructed inline
        # here. That logic now lives entirely inside RealData, honouring the
        # separation-of-concerns principle.
        token_x, token_padding_mask = self.real_data(random_label)
